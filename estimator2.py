...
class TfPoseEstimator:
    ...
    def draw_humans(npimg, humans, imgcopy=False):
        #cv2.imwrite("test1.jpg", npimg)
        if imgcopy:
            npimg = np.copy(npimg)
        image_h, image_w = npimg.shape[:2]
        original_x, original_y =int(image_w/2), int(image_h/2)
        print('image_h, image_w', image_h, image_w )
        centers = {}
        img = np.zeros((image_h,image_w,3), np.uint8) #blank image
        img2 = np.copy(npimg)
        for human in humans:
            # draw point
            for i in range(common.CocoPart.Background.value):
                ...
                #print('body_part.x, body_part.y', body_part.x, body_part.y)
                centers[i] = center
                cv2.circle(npimg, center, 3, (255, 0, 0), thickness=3, lineType=8, shift=0)
                cv2.circle(img, center, 3, (255, 0, 0), thickness=3, lineType=8, shift=0)  
            # draw line
            ...
            for pair_order, pair in enumerate(common.CocoPairsRender):
                ...
                #cv2.line(img, centers[pair[0]], centers[pair[1]], (0, 255, 0), 2)
                
                cv2.line(npimg, centers[pair[0]], (centers[pair[1]][0],centers[pair[0]][1]), (255, 0, 0), 2)
                
                print('fist point',centers[pair[0]])
                print('second point',centers[pair[1]])
                

                #CalAngle
                d01=math.sqrt(math.fabs(centers[pair[1]][0]-centers[pair[0]][0])**2 + math.fabs(centers[pair[1]][1]-centers[pair[0]][1])**2)
                hor=(centers[pair[1]][0]-centers[pair[0]][0])
                ver=(centers[pair[1]][1]-centers[pair[0]][1])

                print('hor', hor)
                print('ver', ver)
                
                cv2.line(img, (original_x, original_y), (int(original_x+hor),int(original_y+ver)), (255, 255, 0), 2)
                cv2.line(img, (0, original_y), (image_w,original_y), (255, 255, 255), 2)
                cv2.line(img, (original_x,0), (original_x ,image_h), (255, 255, 252), 2)
                

                if d01==0:
              	  print('no angle')
                else:
                  angle = np.arcsin(math.fabs(ver)/d01)*180.0/math.pi
                  cv2.putText(npimg, str(int(angle)), centers[pair[0]], cv2.FONT_HERSHEY_COMPLEX, 1, (0, 128, 229), 2)
                  print('angle in degree', angle)
                cv2.imshow("sticks vectors", img)
 	          #cv2.putText(npimg, str(int(angle)), centers[pair[0]], cv2.FONT_HERSHEY_COMPLEX, 1, (0, 128, 229), 2)
                # cv2.line(npimg, centers[pair[0]], centers[pair[1]], common.CocoColors[pair_order], 2)
        #cv2.imwrite("npimg.jpg", npimg)
        #cv2.imwrite("dataset_4_training/walking/img"+str(random.randint(1,100000))+".jpg",img)
        #cv2.rectangle(npimg, (0,0), (400,720), (0,255,0),2);
        return npimg 
    ...
    def get_humans(npimg, humans, imgcopy=False):
        ...
        bboxes = []
        xcenter = []
        # Take out each person's joint point
        for human in humans:
            xs = []
            ys = []
            centers = {}
            # Draw the relevant nodes onto the image
            for i in range(common.CocoPart.Background.value):
                if i not in human.body_parts.keys():
                    continue
            ...
            # Connect joint points belonging to the same person to each part
            for pair_order, pair in enumerate(common.CocoPairsRender):
                if pair[0] not in human.body_parts.keys() or pair[1] not in human.body_parts.keys():
                    continue
