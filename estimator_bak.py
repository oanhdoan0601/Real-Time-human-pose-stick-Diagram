...
class TfPoseEstimator:
    ...
    @staticmethod
    def draw_humans(npimg, humans, imgcopy=False, P_default):
        ...
        centers = {}
        blank_image = np.zeros((image_h,image_w,3), np.uint8) #blank image
        for human in humans:
            # draw point
            for i in range(common.CocoPart.Background.value):
                ...
                cv2.circle(blank_image, center, 3, (0, 0, 255), thickness=4, lineType=8, shift=0)   #################################
            # draw line
            ...
            for pair_order, pair in enumerate(common.CocoPairsRender):
                ...
                cv2.line(blank_image, centers[pair[0]], centers[pair[1]], (0, 255, 0), 1)
                cv2.line(blank_image, P_default, centers[pair[1]], (0, 0, 255), 1)
                
                            
                #cv2.line(npimg, centers[pair[0]], centers[pair[1]], common.CocoColors[pair_order], 2)
        #cv2.imwrite("npimg.jpg", npimg)
        #cv2.imwrite("dataset_4_training/walking/img"+str(random.randint(1,100000))+".jpg",blank_image)
        #cv2.rectangle(npimg, (0,0), (400,720), (0,255,0),2);
        #return npimg
        return blank_image, centers[pair[0]]

    @staticmethod
    def get_humans(npimg, humans, imgcopy=False, Point_default):
        #global Point_default
        if imgcopy:
            npimg = np.copy(npimg)
        image_h, image_w = npimg.shape[:2]
        joints = []
